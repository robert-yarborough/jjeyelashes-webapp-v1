describe('Card Flip Directive', function() {
    ...
    it('Should use height 100% of containing element for unflipped card if height not defined', function(){
        //Arrange
        var fixture = setFixtures('<div id="parent-div">'+
    ...
    it('Should work regularly if image is not defined', function(){
        //Arrange
        $scope = $rootScope.$new();
        var fixture = setFixtures('<fmp-card suffix="1" front-caption="Card front caption" small-card-width="120px" >' +
            '<div id="card-back1">Card Back 1</div>' +
            '</fmp-card>');
    ...
    it('Should work regularly if caption is not defined', function(){
        //Arrange
        $scope = $rootScope.$new();
        var fixture = setFixtures('<fmp-card suffix="1" image="card-front-icon.png" small-card-width="120px">' +
            '<div id="card-back1">Card Back 1</div>' +
            '</fmp-card>');
    ...
    it('Should set small unflipped card width according to width param', function() {
        ...
        var fixture = setFixtures(
            '<fmp-card suffix="1" image="card-front-icon.png" front-caption="Card front caption" small-card-width="98px">' +
            '<div id="card-back1">Card Back 1</div>' +
            '</fmp-card>');
    ...
    it('Should set set small unflipped card suffix according to width param', function() {
        ...
        $scope = $rootScope.$new();
        var fixture = setFixtures(
            '<fmp-card suffix="' + expectedIdSuffix + '" image="card-front-icon.png" front-caption="Card front caption" small-card-width="98px">' +
            '<div id="card-back1">Card Back 1</div>' +
            '</fmp-card>');
    ...
    it('Should set small unflipped card background src according to width param', function() {
        ...
        $scope = $rootScope.$new();
        var fixture = setFixtures(
            '<fmp-card suffix="test" image="' + expectedBackgroundSrc + '" front-caption="Card front caption" small-card-width="98px">' +
            '<div id="card-back1">Card Back 1</div>' +
            '</fmp-card>');
    ...
    it('Should flip small card once clicked to large representation', function() {
        ...
        $scope = $rootScope.$new();
        var fixture = setFixtures(
            '<fmp-card suffix="test" image="card-front-icon.png" front-caption="' + expectedCaption + '" small-card-width="98px"  card-control="flipper">' +
            '<div id="card-back1">Card Back 1</div>' +
            '</fmp-card>');
        $scope.flipper = {};
        $compile($("body"))($scope);
    ...
    it('Should close flipped large card once clicked', function(done) {
        ...
        $scope = $rootScope.$new();
        var fixture = setFixtures(
            '<fmp-card suffix="test" image="card-front-icon.png" front-caption="' + expectedCaption + '" small-card-width="98px" >' +
            '<div id="card-back1">Card Back 1</div>' +
            '</fmp-card>');
        $compile($("body"))($scope);
        $scope.$digest();
    ...
    it('Should fire onCardOpen event when clicking on small card', function(done) {
        ...
        var fixture = setFixtures(
            '<fmp-card suffix="test" image="card-front-icon.png" front-caption="' + expectedCaption + '" small-card-width="98px" on-card-opened="onCardOpen()">' +
            '<div id="card-back1">Card Back 1</div>' +
            '</fmp-card>');
        $compile($("body"))($scope);
        $scope.$digest();
    ...
    it('Should fire onCardClose event when clicking on small card and cancel close if return value is false', function(done) {
        ...
        var fixture = setFixtures(
            '<fmp-card suffix="test" image="card-front-icon.png" front-caption="' + expectedCaption + '" small-card-width="98px" on-card-closed="onCardClose()">' +
            '<div id="card-back1">Card Back 1</div>' +
            '</fmp-card>');
        $compile($("body"))($scope);
        $scope.$digest();
